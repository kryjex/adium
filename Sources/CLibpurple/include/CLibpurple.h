...
typedef void (*fluorite_purple_on_contact_cb)(const char* name, const char* handle, const char* status_id, const char* status_name, const char* group, const char* protocol_id);
...
typedef void (*fluorite_purple_on_message_cb)(const char* sender_handle, const char* message_text, bool is_from_me, bool is_system, const char* protocol_id, const char* account_username, long long timestamp, const void* image_data, size_t image_size);
typedef void (*fluorite_purple_on_status_cb)(const char* status_text);
typedef void (*fluorite_purple_on_account_state_cb)(const char* username, const char* protocol_id, bool is_connected, const char* status_msg);

// Extended callback function signatures for interactive requests, connection progress, typing & buddy removal
typedef void (*fluorite_purple_on_request_input_cb)(void* request_handle, const char* title, const char* primary, const char* secondary, const char* default_value, bool masked, const char* hint);
typedef void (*fluorite_purple_on_request_action_cb)(void* request_handle, const char* title, const char* primary, const char* secondary, int default_action, const char* const* action_titles, int action_count);
typedef void (*fluorite_purple_on_request_close_cb)(void* request_handle);
typedef void (*fluorite_purple_on_connection_progress_cb)(const char* username, const char* protocol_id, const char* text, size_t step, size_t step_count);
typedef void (*fluorite_purple_on_typing_cb)(const char* handle, bool is_typing);
typedef void (*fluorite_purple_on_buddy_removed_cb)(const char* handle);
typedef void (*fluorite_purple_on_notify_message_cb)(int type, const char* title, const char* primary, const char* secondary);

// File transfer (PurpleXfer) callback signatures
typedef void (*fluorite_purple_on_xfer_new_cb)(void* xfer_handle, const char* who, const char* filename, size_t size, bool is_incoming);
typedef void (*fluorite_purple_on_xfer_update_cb)(void* xfer_handle, size_t bytes_sent, size_t bytes_total, int status);
typedef void (*fluorite_purple_on_xfer_cancel_cb)(void* xfer_handle, bool by_local);
// Libpurple is about to free the PurpleXfer. Do not use the raw pointer after this callback fires.
typedef void (*fluorite_purple_on_xfer_destroyed_cb)(void* xfer_handle);

// Group chat (MUC) callback signatures
typedef void (*fluorite_purple_on_chat_joined_cb)(const char* room_name, const char* username, const char* protocol_id);
typedef void (*fluorite_purple_on_chat_left_cb)(const char* room_name, const char* username, const char* protocol_id);
typedef void (*fluorite_purple_on_chat_message_cb)(const char* room_name, const char* account_username, const char* protocol_id, const char* sender, const char* message_text, bool is_from_me, bool is_system, long long timestamp);
...
typedef void (*fluorite_purple_on_chat_buddy_joined_cb)(const char* room_name, const char* buddy_name, bool new_arrival);
typedef void (*fluorite_purple_on_chat_buddy_left_cb)(const char* room_name, const char* buddy_name);
...
typedef void (*fluorite_purple_on_chat_listed_cb)(const char* room_name, const char* title, const char* group_name, const char* username, const char* protocol_id);
typedef void (*fluorite_purple_on_chat_unlisted_cb)(const char* room_name, const char* username, const char* protocol_id);
